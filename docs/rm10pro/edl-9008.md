...
- **Programming partitions** — works, with two gotchas:
  ...
  - Pair `init_boot` flashes with a `vbmeta` flag patch (set offset 0x0C to `02 00 00 00`) — otherwise AVB rejects boot. See [Partitions, AVB, vbmeta](/rm10pro/partitions-avb).
...
| "Your device is corrupt" after flash | vbmeta not patched | See [Partitions, AVB, vbmeta](/rm10pro/partitions-avb) |

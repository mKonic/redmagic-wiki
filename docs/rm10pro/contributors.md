...
| **MrKonic** | 2 (#671, #672) | **You.** Forked Redmagic-Control-Center for RM10 Pro; announced NX789S kernel source availability. |
...
The RM11 Pro / ZTE Family Toolbox thread (`rm11pro-unlock.md`) has its own cast. Notable overlap:
- **AdaUnlocked** drives the rallying / news there too.
- **SYXZ** is the OP / tool author.
- Toolbox technical questions usually get more authoritative answers in the RM11 thread.

## Locating a person's posts

```bash
grep -n "^### #" extracted/rm10pro.md | grep "Ssmiles" | head
# or
~/.venvs/xda/bin/python -c "
import json
for p in (json.loads(l) for l in open('extracted/rm10pro.jsonl')):
